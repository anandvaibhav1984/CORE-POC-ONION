...
namespace cBaseQms.Core.Repositry.Repositories
{
    public class AppParameterRepository: RepositoryBase<AppParameter>, IAppParameterRepository
    {
        ...
        {
            var appParam = this.DbContext.AppParameter.Where(x => x.ParamName == paramName).FirstOrDefault();
        ...
        {
            var appParam = this.DbContext.AppParameter.Where(x => x.AppParamId == id).FirstOrDefault();
            return appParam;
        }

        public List<AppParameter> GetParameters()
        {
            var appParam = this.DbContext.AppParameter.ToList();
            return appParam;
        }
